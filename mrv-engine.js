...
async function iniciarApp() {
    // Define o título solicitado
    const topo = document.querySelector('.topo-dashboard');
    if (topo) topo.innerText = "RESIDENCIAIS MRV EM SÃO PAULO";
...
async function carregarPlanilha() {
    ...
    try {
        ...
        // CORREÇÃO: Garante que a última linha da planilha seja lida
        if (!texto.endsWith('\n')) texto += '\n';
...
function comandoSelecao(idPath, nomePath, fonte) {
    ...
    if (imoveis.length > 0) {
        ...
        // Ativa o botão correspondente na lista lateral
...
function montarVitrine(selecionado, listaDaCidade, nomeRegiao) {
    ...
    let html = `<div class="vitrine-topo">MRV EM ${nomeRegiao.toUpperCase()}</div>
                ...
                <p style="font-size:0.65rem; color:#444; margin-bottom:8px; display:flex; justify-content:space-between; align-items:center;">
                    ...
                    <a href="${urlMaps}" target="_blank" class="btn-maps">VER NO MAPA</a>
                </p>`;
